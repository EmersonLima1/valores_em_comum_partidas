...
def gerar_resultados():
  ...
  for sheet_name in sheet_names:
      ...
      for index, row in df.iterrows():
          ...
          # Verificar se o valor dentro dos parênteses é maior que 80%
          if float(total_am.split('(')[1].split(')')[0][:-1]) > 80:
      ...
      valores_int = [int(valor) for valor in partidas_apos_lista if int(valor) >= 8]
      ...
      for valor_partida in valores_int:
        ...
        chaves_aleatorias = random.sample(list(ambas_marcaram_transformado.keys()), 10)
        for chave in chaves_aleatorias:
            ambas_marcaram_anterior[chave] = ambas_marcaram_transformado[chave]

        # Criação de ambas_marcaram_anterior
        ambas_nao_marcaram_anterior = {}
        chaves_aleatorias = random.sample(list(ambas_nao_marcaram_transformado.keys()), 10)
        for chave in chaves_aleatorias:
            ambas_nao_marcaram_anterior[chave] = ambas_nao_marcaram_transformado[chave]
        ...
        for chave in ambas_marcaram_anterior:
            if chave in ambas_marcaram_anterior:
                ...
                ambas_marcaram_partidas_anterior[chave] = [primeira_partida, segunda_partida, terceira_partida, quarta_partida, quinta_partida, sexta_partida, setima_partida]

        # Verificação das chaves em ambas_nao_marcaram_anterior
        for chave in ambas_nao_marcaram_anterior:
            if chave in ambas_nao_marcaram_anterior:
                lista_partidas_an = ambas_nao_marcaram_anterior[chave]
                primeira_partida = lista_partidas_an[valor_partida - 2][0]
                segunda_partida = lista_partidas_an[valor_partida - 3][0]
                terceira_partida = lista_partidas_an[valor_partida - 4][0]
                quarta_partida = lista_partidas_an[valor_partida - 5][0]
                quinta_partida = lista_partidas_an[valor_partida - 6][0]
                sexta_partida = lista_partidas_an[valor_partida - 7][0]
                setima_partida = lista_partidas_an[valor_partida - 8][0]

                ambas_nao_marcaram_partidas_anterior[chave] = [primeira_partida, segunda_partida, terceira_partida, quarta_partida, quinta_partida, sexta_partida, setima_partida]

        ambas_marcaram_partidas_anterior = dict(sorted(ambas_marcaram_partidas_anterior.items()))
        ambas_nao_marcaram_partidas_anterior = dict(sorted(ambas_nao_marcaram_partidas_anterior.items()))
        ...
        valores = []
        for i in range(num_colunas, 0, -1):  # Percorre de 7 a 1 em ordem decrescente
        ...
        df_ambas_nao = pd.DataFrame(columns=colunas)

        # Preenchimento do DataFrame
        for chave, valores in ambas_nao_marcaram_partidas_anterior.items():
            dados_chave = []
            for valor in valores:
                am_an, resultado, gols, primeiro_tempo = valor.split(' - ')
                dados_chave.extend([am_an, resultado, gols, primeiro_tempo])
            df_ambas_nao.loc[chave] = dados_chave

        df_ambas_nao_transposto = df_ambas_nao.transpose()
        df_ambas_nao_transposto = df_ambas_nao_transposto[::-1]
        ...
        # Adicione a nova coluna "Partidas atrás" ao dataframe
        df_ambas_nao_transposto.insert(0, "Partidas atrás", valores)

        df_ambas_nao_transposto = df_ambas_nao_transposto.reset_index(drop=True)
        ...
        def encontrar_valores_comuns(dataframe):
            ...
            for i in range(len(dataframe)):
                ...
                if "Gols" in linha[0]:
                    ...
                    # Verifica se todos os valores da linha são iguais
                    if all(x == linha[0] for x in linha):
                        valores_comuns.append([linha[0]])  # Adiciona o valor comum à lista de valores comuns
                    else:
                        valores_comuns.append(None)  # Se os valores não forem todos iguais, adiciona None à lista

            return valores_comuns


        # Obtendo os valores para df_ambas_nao_transposto e df_ambas_transposto
        valores_nao_marcaram = encontrar_valores_comuns(df_ambas_nao_transposto)
        valores_marcaram = encontrar_valores_comuns(df_ambas_transposto)

        # Criando o DataFrame final
        df_result = pd.DataFrame({
            'Linhas': [f'Valores comuns na linha {i}' for i in range(len(valores_nao_marcaram))],
            'Ambas marcaram': valores_marcaram,
            'Ambas não marcaram': valores_nao_marcaram
        })

        # Sequência de índices desejada
        sequencia_indices = [24, 25, 26, 27, 20, 21, 22, 23, 16, 17, 18, 19, 12, 13, 14, 15, 8, 9, 10, 11, 4, 5, 6, 7, 0, 1, 2, 3]
        ...
        st.write('Verificando '+ valor_partida + ' após a ocorrência do padrão, esses foram os valores em comum em 10 partidas que deram certo e em 10 partidas que deram errado')
        st.write(df_result)
